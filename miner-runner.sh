#!/usr/bin/env bash
# Internal supervisor for a capped miner. Not a cluster CLI entry point.
set -euo pipefail
umask 077
MODE=${1:-}; shift || true
same_process() {
...
[[ "$CPU" =~ ^([1-9]|[1-9][0-9]|100)$ ]] || exit 2
[ -x "$LIMITER" ] && [ -x "$CKB" ] && [ -d "$DIR" ] || exit 2
cleanup() {
  local rc=$?
...
while same_process "$DIR/miner-worker.pid"; do
  if ! same_process "$DIR/miner-limiter.pid"; then
